...
function RichTextCreator:_CreateRichElement(richText, strValue, size, color)
    ...
    for _, value in pairs(strValue.strValues) do
        if value == "\n" then
            ...
            richText:pushBackElement(createElement(value, size, color))
        end
    end

end

function RichTextCreator:_CreateRichTextWithAnything(value, width, defaultSize, defaultColor)
    ...
    for _, value in pairs(elements) do 
        ...
        local url = value.attributes.url
